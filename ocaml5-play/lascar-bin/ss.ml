...
module Int = struct
  include Builtins.Int

  let epsilon = 0
end

module String = struct
  include Builtins.String
...
let a1 =
  A.create ~states:[ 0; 1; 2 ] ~symbols:[ "a"; "b" ] ~istate:0
    ~trans:[ (0, "a", 0); (0, "b", 1); (1, "a", 0); (1, "b", 2) ]
    ~astates:[ 2 ]
...
let trans a q s =
  ...
  with A.Stuck (q, s) ->
    Printf.printf "** a1: no transition from %s with %s\n"
      (Builtins.Int.to_string q)
      (ListExt.to_string Fun.id "," s);
    0
