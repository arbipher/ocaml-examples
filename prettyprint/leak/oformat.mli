...
(** Pretty-printing.
    ...
    {{!boxes} 'pretty-printing boxes'} and {{!tags} 'semantic tags'} combined
    with a set of {{!fpp} printf-like functions}. The pretty-printer splits
    lines at specified {{!breaks} break hints}, and indents lines according to
    the box structure. Similarly, {{!tags} semantic tags} can be used to
    decouple text presentation from its contents.
    ...
    functions. Some formatters are predefined, notably:
    ...
    {b Warning}: Since {{!section:formatter} formatters} contain mutable state,
    it is not thread-safe to use the same formatter on multiple domains in
    parallel without synchronization.

    If multiple domains write to the same output channel using the predefined
    formatters (as obtained by {!get_std_formatter} or {!get_err_formatter}),
    the output from the domains will be interleaved with each other at points
    where the formatters are flushed, such as with {!print_flush}. This
    synchronization is not performed by formatters obtained from
    {!formatter_of_out_channel} (on the standard out channels or others). *)
...
(** {1 Introduction}
    ...
    pretty-printing box. Each box opened by one of the [open_] functions below
    must be closed using [close_box] for proper formatting. Otherwise, some of
    ...
    to {!Stdlib} low level output functions is error prone.

    The pretty-printing functions output material that is delayed in the
    pretty-printer queue and stacks in order to compute proper line splitting.
    In contrast, basic I/O output functions write directly in their output
    device. As a consequence, the output of a basic I/O function may appear
    before the output of a pretty-printing function that has been called before.
    For instance,
    [
    Stdlib.print_string "<";
...
type stag +=
  | String_tag of tag
        (** [String_tag s] is a string tag [s]. String tags can be inserted
            either by explicitly using the constructor [String_tag] or by using
            the dedicated format syntax ["@{<s> ... @}"].
...
(** The pretty-printing engine uses the concepts of pretty-printing box and
    ...
    - within an {e horizontal/vertical} box, if the box fits on the current line
      ...
      the line,
    - within a {e compacting} box, a break hint never splits the line, unless
...
(** [pp_open_box ppf d] opens a new compacting pretty-printing box with offset
    [d] in the formatter [ppf].
...
(** A 'break hint' tells the pretty-printer to output some space or split the
    ...
    pretty-printing engine, since those notions can be completely redefined by
    the programmer. However, in the pretty-printer default setting, ``output a
...
(** End of pretty-printing: resets the pretty-printer to initial state.
    ...
    repeated calls to [print_flush] means repeated calls to {!Stdlib.flush} to
    flush the out channel; these explicit flush calls could foil the buffering
...
(** End of pretty-printing: resets the pretty-printer to initial state.
    ...
    low-level output device immediately before the device is flushed. See
    corresponding words of caution for {!print_flush}.
...
(** [pp_set_margin ppf d] sets the right margin to [d] (in characters): the
    ...
    break hints given. Setting the margin to [d] means that the formatting
    engine aims at printing at most [d-1] characters per line. Nothing happens
    if [d] is smaller than 2. If [d] is too large, the right margin is set to
    the maximum admissible value (which is greater than [10 ^ 9]). If [d] is
    less than the current maximum indentation limit, the maximum indentation
    limit is decreased while trying to preserve a minimal ratio
    [max_indent/margin>=50%] and if possible the current difference
    [margin - max_indent].
...
(** [pp_set_max_indent ppf d] sets the maximum indentation limit of lines to [d]
    ...
    limit ([7>5]) and its parent box does not fit on the current line. Either
    decreasing the length of the parent box to make it fit on a line:
    ...
    ["123456789"] and ["123456789A"] . Note also that vertical boxes never fit
    on a line whereas horizontal boxes always fully fit on the current line.
    Opening a box may split a line whereas the contents may have fit. If this
    behavior is problematic, it can be curtailed by setting the maximum
...
(** A {e tabulation box} prints material on lines divided into cells of fixed
    ...
    use of tabulation boxes allows simple printing of columns within module
    {!Format}. *)
...
(** [open_tbox ()] opens a new tabulation box.
    ...
    {!Format.set_tab} sets a tabulation marker at insertion point.
...
(** Set the text of the ellipsis printed when too many pretty-printing boxes are
    open (a single dot, [.], by default). *)
...
(* type stag = .. *)
(** {i Semantic tags} (or simply {e tags}) are user's defined annotations to
    ...
    like parentheses using {!pp_open_stag} and {!pp_close_stag}.
    ...
    pretty-printing routine can output both simple 'verbatim' material or richer
    decorated output depending on the treatment of tags. By default, tags are
    not active, hence the output is not decorated with tag information. Once
    [set_tags] is set to [true], the pretty-printer engine honors tags and
    decorates the output accordingly.
    ...
    Semantic tag operations may be set on or off with {!set_tags}. Tag-marking
    operations may be set on or off with {!set_mark_tags}. Tag-printing
    operations may be set on or off with {!set_print_tags}.
...
(** Redirect the standard pretty-printer output to the given channel. (All the
    ...
    [set_formatter_out_channel] is equivalent to {!pp_set_formatter_out_channel}
    [std_formatter]. *)
...
(** [pp_set_formatter_output_functions ppf out flush] redirects the standard
    ...
    called with a string [s], a start position [p], and a number of characters
    ...
    conversion [%!], or pretty-printing indications [@?] or [@.], or using low
...
(** The [Format] module is versatile enough to let you completely redefine the
    meaning of pretty-printing output: you may provide your own functions to
    define how to handle indentation, line splitting, and even printing of all
    the characters that have to be printed! *)
...
(** The set of output functions specific to a formatter:
    ...
    - fields [out_string] and [out_flush] are output device specific; (e.g.
      {!Stdlib.output_string} and {!Stdlib.flush} for a {!Stdlib.out_channel}
      device, or [Buffer.add_substring] and {!Stdlib.ignore} for a [Buffer.t]
      output device),
...
(** [pp_set_formatter_stag_functions ppf tag_funs] changes the meaning of
    ...
    next call to [close_stag ()] happens, the semantic tag name [t] is sent back
    to the closing tag-marking function (the [mark_close_stag] field of record
...
(** {1:formatter Defining formatters}
    ...
    pretty-printing boxes simultaneously open, ellipsis, and so on, are specific
    to each formatter and may be fixed independently.

    For instance, given a {!Buffer.t} buffer [b], {!formatter_of_buffer} [b]
    returns a new formatter using buffer [b] as its output device. Similarly,
    given a {!Stdlib.out_channel} output channel [oc],
    {!formatter_of_out_channel} [oc] returns a new formatter using channel [oc]
...
(** [synchronized_formatter_of_out_channel oc] returns the key to the
    ...
    flushed, such as with {!print_flush}. *)
...
(** [formatter_of_buffer b] returns a new formatter writing to buffer [b]. At
    the end of pretty-printing, the formatter must be flushed using
    {!pp_print_flush} or {!pp_print_newline}, to print all the pending material
    into the buffer. *)
...
(** [formatter_of_out_functions out_funs] returns a new formatter that writes
    ...
    See definition of type {!formatter_out_functions} for the meaning of
    argument [out_funs].
...
(** [pp_print_list ?pp_sep pp_v ppf l] prints items of list [l], using [pp_v] to
    ...
    {!pp_print_cut}). Does nothing on empty lists.
...
val pp_print_array :
  ...
  'a array ->
  unit
(** [pp_print_array ?pp_sep pp_v ppf a] prints items of array [a], using [pp_v]
    to print each item, and calling [pp_sep] between items ([pp_sep] defaults to
    {!pp_print_cut}). Does nothing on empty arrays.

    If [a] is mutated after [pp_print_array] is called, the printed values may
    not be what is expected because [Format] can delay the printing. This can be
...
(** [pp_print_seq ?pp_sep pp_v ppf s] prints items of sequence [s], using [pp_v]
    to print each item, and calling [pp_sep] between items ([pp_sep] defaults to
    {!pp_print_cut}. Does nothing on empty sequences.
...
val pp_print_result :
  ...
  ('a, 'e) result ->
  unit
(** [pp_print_result ~ok ~error ppf r] prints [r] on [ppf] using [ok] if [r] is
    [Ok _] and [error] if [r] is [Error _].
...
val pp_print_either :
  ...
  ('a, 'b) Either.t ->
  unit
(** [pp_print_either ~left ~right ppf e] prints [e] on [ppf] using [left] if [e]
    is [Either.Left _] and [right] if [e] is [Either.Right _].
...
(** [fprintf ff fmt arg1 ... argN] formats the arguments [arg1] to [argN]
    ...
    {!Printf} module, and pretty-printing indications specific to the [Format]
    module.
    ...
    - [@\[]: open a pretty-printing box. The type and offset of the box may be
      ...
      offset, and the closing [>] character. Pretty-printing box type is one of
      [h], [v], [hv], [b], or [hov]. '[h]' stands for an 'horizontal'
      ...
      stands for an 'horizontal-or-vertical' pretty-printing box demonstrating
      indentation, '[hov]' stands a simple 'horizontal-or-vertical'
      pretty-printing box. For instance, [@\[<hov 2>] opens an
      'horizontal-or-vertical' pretty-printing box with indentation 2 as
      obtained with [open_hovbox 2]. For more details about pretty-printing
    ...
    - [@;]: output a 'full' break hint as with [print_break]. The [nspaces] and
      ...
      following syntax: the [<] character, followed by an integer [nspaces]
      value, then an integer [offset], and a closing [>] character. If no
      parameters are provided, the full break defaults to a 'space' break hint.
    - [@.]: flush the pretty-printer and split the line, as with
      [print_newline ()].
    - [@<n>]: print the following item as if it were of length [n]. Hence,
      [printf "@<0>%s" arg] prints [arg] as a zero length string. If [@<n>] is
      not followed by a conversion specification, then the following character
    ...
    - [@\{]: open a semantic tag. The name of the tag may be optionally
      ...
      optional string specification, and the closing [>] character. The string
    ...
    indication, escape it with a [%] character. Old quotation mode [@@] is
    deprecated since it is not compatible with formatted input interpretation of
    character ['@'].
...
(** Same as [printf] above, but instead of printing on a formatter, returns a
    ...
    pretty-printer queue is flushed at the end of {e each call} to [sprintf].
    Note that if your format string contains a [%a], you should use [asprintf].
...
(** Same as {!fprintf}, except the formatter is the last argument.
    ...
    This can be used as a replacement for {!asprintf} to delay formatting
    decisions. Using the string returned by {!asprintf} in a formatting context
    forces formatting decisions to be taken in isolation, and the final string
    may be created prematurely. {!dprintf} allows delay of formatting decisions
...
(** Same as {!dprintf} above, but instead of returning immediately, passes the
...
(** {1:examples Examples}
    ...
    We have a list [l] of pairs [(int * bool)], which the toplevel prints for
    us:
    ...
    - [pp_pair] prints a pair [bool*int] surrounded in "(" ")". It takes a
      formatter (into which formatting happens), and the pair itself. When
      printing is done it returns [()].

    - [Format.printf "l = \[@\[<hov>%a@\]\]@." ... l] is like [printf], but with
      additional formatting instructions (denoted with "\@"). The pair
    ...
    - "%a" is a formatting instruction, like "%d" or "%s" for [printf]. However,
      where "%d" prints an integer and "%s" prints a string, "%a" takes a
      printer (of type [Format.formatter -> 'a -> unit]) and a value (of type
      ['a]) and applies the printer to the value. This is key to
      compositionality of printers.

    - We build a list printer using
      [Format.pp_print_list ~pp_sep:(...) pp_pair]. [pp_print_list] takes an
      element printer and returns a list printer. The [?pp_sep] optional
      argument, if provided, is called in between each element to print a
      separator.

    - Here, for a separator, we use [(fun out () -> Format.fprintf out ";@ ")].
      It prints ";", and then "\@ " which is a breaking space (either it prints
      " ", or it prints a newline if the box is about to overflow). This "\@ "
      is responsible for the list printing splitting into several lines.
    ...
    in your program. If, for example, you were to define basic geometry types
    like so:
    ...
    it would be convenient to define printers for these types. Here is an
    example of to do it. Note that "%.3f" is a [float] printer up to 3 digits of
    precision after the dot; "%f" would print as many digits as required, which
    ...
    {[
      ...
      let pp_rectangle out (r : rectangle) =
        Format.fprintf out "{ @[ll=%a;@ ur=%a@] }" pp_point r.ll pp_point r.ur
    ]}
    ...
    {{:https://caml.inria.fr/resources/doc/guides/format.en.html} "Using the
      Format module"}.

    A final note: the [Format] module is a starting point. The OCaml ecosystem
    has libraries that makes formatting easier and more expressive, with more
    combinators, more concise names, etc. An example of such a library is
    {{:https://erratique.ch/software/fmt} Fmt}.

    Automatic deriving of pretty-printers from type definitions is also
    possible, using
    {{:ppx_deriving.show} https://github.com/ocaml-ppx/ppx_deriving} or similar
    ppx derivers. *)
